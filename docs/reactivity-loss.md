...
определение [render prop](https://reactjs.org/docs/render-props.html). После появления хуков такие ситуации встречаются
...
В этом примере `MyForm` это observer, но функция, передающаяся как проп `render`, не является, а значит
